...
const assertDraftVersion = (version: { status: string; is_published: boolean }) => {
	if (version.is_published || version.status === "published") {
		throw new AppError(400, "Published versions are immutable");
	}

	if (version.status !== "draft") {
		throw new AppError(400, "Fields can only be modified while the version is in draft status");
	}
};
...
export class FormTemplatesService {
	async createTemplate(input: CreateTemplateInput, user: AuthenticatedUser) {
		...
		if (input.source_document_id) {
			...
			if (sourceDocument.org_id !== orgId) {
				throw new AppError(400, "Source document belongs to another organization");
			}
		}
	...
	async createVersion(templateId: string, input: CreateVersionInput, user: AuthenticatedUser) {
		const template = await getTemplateByIdInternal(templateId);
	...
	async updateVersion(versionId: string, input: UpdateVersionInput, user: AuthenticatedUser) {
		const versionWithTemplate = await getTemplateWithOrgByVersionId(versionId);
		...
		if (!input.status) {
			throw new AppError(400, "No form template version fields provided for update");
		}

		if (!VERSION_STATUSES.has(input.status)) {
			throw new AppError(400, "Invalid status update. Use publish endpoint to publish a version");
		}
	...
	async addFieldToVersion(versionId: string, input: AddFieldInput, user: AuthenticatedUser) {
		const versionWithTemplate = await getTemplateWithOrgByVersionId(versionId);
		...
		if (!label || !inputType) {
			throw new AppError(400, "label and input_type are required when field_catalog_id is not provided");
		}
		...
		const [createdField] = (await db("form_fields")
			.insert({
				...
				options_json: toJson(
					input.options_json !== undefined ? input.options_json : fromJson(fieldCatalog?.options_json),
				),
	...
	async updateField(fieldId: string, input: UpdateFieldInput, user: AuthenticatedUser) {
		...
		if (input.field_catalog_id) {
			const catalog = await db("field_catalog").where({ id: input.field_catalog_id }).first();
			if (!catalog) {
				throw new AppError(404, "Field catalog entry not found");
			}
		}
	...
	async publishVersion(versionId: string, user: AuthenticatedUser) {
		...
		await db.transaction(async (trx) => {
			await trx("form_template_versions")
				.where({ template_id: versionWithTemplate.template_id, is_published: true })
				.update({
					is_published: false,
					status: "archived",
					updated_at: new Date(),
			...
			await trx("form_templates").where({ id: versionWithTemplate.template_id }).update({
				status: "active",
				updated_at: new Date(),
			});
		});

		const published = (await db("form_template_versions").where({ id: versionId }).first()) as
			| FormTemplateVersionRow
			| undefined;
	...
	async createTemplateFromDocument(
		...
		const document = (await db("documents").where({ id: documentId }).first()) as
			| DocumentRow
			| undefined;
			...
			if (rawExtraction?.extracted_fields?.length) {
				...
				if (extractedKeys.length > 0) {
					const catalogRows = (await db("field_catalog")
						...
						.select("id", "key", "name", "input_type", "options_json")) as FieldCatalogRow[];
		...
		const existingCatalogRows = requestedCatalogIds.length
			? ((await db("field_catalog").whereIn("id", requestedCatalogIds).select("id")) as {
					id: string;
			  }[])
			: [];
		const existingCatalogIds = new Set(existingCatalogRows.map((row) => row.id));

		const normalizedFields = extractedFields.map((field) => {
			const fieldCatalogId = field.fieldCatalogId && existingCatalogIds.has(field.fieldCatalogId)
		...
		const result = await db.transaction(async (trx) => {
			const templateName = input.name?.trim() || `Draft from ${document.file_name}`;
		...
		return {
			...
			summary: {
				...
				catalogMappedFields: result.fields.filter((field) => !field.isCustom).length,
			},
		};
	}
}
