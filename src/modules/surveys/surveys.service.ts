...
const mapSurveyResponse = (response: SurveyResponseRow) => {
  return {
    ...
    valueNumber:
      response.value_number === null ? null : Number(response.value_number),
...
const getSurveyRowById = async (surveyId: string) => {
  return (await db("surveys").where({ id: surveyId }).first()) as
    | SurveyRow
    | undefined;
};
...
const assertResponseMatchesField = (
...
const maybePushNeed = (needs: NeedDraft[], need: NeedDraft) => {
  if (
    needs.some(
      (item) =>
        item.category === need.category && item.summary === need.summary,
    )
  ) {
    return;
  }
...
const buildNeedsFromResponses = (
  ...
  const urgentAssistance = fromJson(
    byKey.get("urgent_assistance_required")?.value_json,
  );
  ...
  if (
    ...
    maybePushNeed(needs, {
      ...
      urgencyScore:
        waterAccess === "river" || waterAccess === "none" ? 0.88 : 0.76,
      priorityLevel:
        waterAccess === "river" || waterAccess === "none" ? "high" : "medium",
      skillKeys: ["sanitation_support", "water_testing"],
    });
  }

  if (Array.isArray(urgentAssistance)) {
    const assistanceValues = urgentAssistance.filter(
...
export class SurveysService {
  async createSurvey(input: CreateSurveyInput, user: AuthenticatedUser) {
    const templateVersion = await getTemplateVersionContext(
    ...
    if (
      !templateVersion.is_published ||
      templateVersion.status !== "published"
    ) {
      throw new AppError(
        409,
        "Survey must use a published form template version",
      );
    }
    ...
    if (templateVersion.template_org_id !== orgId) {
      throw new AppError(
        403,
        "Template version belongs to another organization",
      );
    }
  ...
  async listSurveys(query: ListSurveysQuery, user: AuthenticatedUser) {
    ...
    const baseQuery = db("surveys as s").leftJoin(
      "form_template_versions as v",
  ...
  async submitSurvey(
    ...
    const fieldsById = new Map(
      templateFields.map((field) => [field.id, field]),
    );
    ...
    for (const response of input.responses) {
      ...
      if (!field) {
        throw new AppError(
          422,
          `Form field ${response.form_field_id} does not belong to this survey template`,
        );
      }
      ...
      if (seenFieldIds.has(response.form_field_id)) {
        throw new AppError(
          422,
          `Duplicate response submitted for field ${field.label}`,
        );
      }
    ...
    for (const field of templateFields) {
      ...
      const response = input.responses.find(
        (item) => item.form_field_id === field.id,
      );
      if (!response || !hasMeaningfulValue(field.input_type, response)) {
        throw new AppError(
          422,
          `Required field ${field.label} is missing a value`,
        );
      }
    }
    ...
    await db.transaction(async (trx) => {
      ...
      await auditService.writeEvent(trx, {
        ...
        newValue: {
          ...
          responseFieldIds: input.responses.map(
            (response) => response.form_field_id,
          ),
        },
  ...
  async analyzeNeeds(surveyId: string, user: AuthenticatedUser) {
    ...
    if (survey.status === "draft") {
      throw new AppError(
        409,
        "Draft surveys must be submitted before needs analysis",
      );
    }
    ...
    console.info(`\n============== AI SURVEY REVIEW ==============`);
    console.info(
      `[SurveyReview] Admin initiated AI Need Synthesis on Submitted Survey!`,
    );
    console.info(
      `[SurveyReview] Survey ID: ${surveyId} | Respondent: ${survey.respondent_name}`,
    );
    console.info(
      `[SurveyReview] Submitting ${responses.length} responses for Vertex AI orchestration...`,
    );

    const fallbackDrafts = buildNeedsFromResponses(survey, responses);
    const uniqueSkillKeys = Array.from(
      new Set(fallbackDrafts.flatMap((draft) => draft.skillKeys)),
    );
    const skillRows = uniqueSkillKeys.length
      ? ((await db("skills")
          .whereIn("key", uniqueSkillKeys)
          .select("id", "key", "name")) as SkillRow[])
      : [];
    ...
    console.info(`\n[SurveyReview] Vertex Synthesis Completed!`);
    console.info(
      `[SurveyReview] Total valid Needs generated: ${drafts.length}`,
    );
    console.info(
      `==========================================================\n`,
    );
  ...
  async deleteSurvey(surveyId: string): Promise<void> {
    const result = await db("surveys").where("id", surveyId).del();
    if (result === 0) {
      throw new AppError(404, "Survey not found");
    }
  }
